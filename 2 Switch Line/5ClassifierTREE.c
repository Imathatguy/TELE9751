...
// initialising root node and temp nodes for contructing routing tables
treeArray *root = NULL;
void testData();
...
int main() {
    ...
    clasIN_addr.sin_port = htons(INPUT_PORT);			// Using port 5000
    ...
    clasOUT_addr.sin_port = htons(OUTPUT_PORT);					// output port 5001
	
    // Binding above data (address/port/etc) to socket descriptor sock for port 5000
    if((bind(sockIN, (struct sockaddr *)&clasIN_addr, sizeof(clasIN_addr))) < 0)
    	DieWithError("bind() failed");
    // -----------End Network Code-------------------------
    
    testData();		// setup random tree table
	...
	//display_database(root, 0);	// Display address database
...
int search(char ip_addr[4][9]) {
	int n=0, i=0, count=0;	// i is number of tiers searched too, where 0 is root and 4 is 4th number of IP, count keeps track of how many steps in search
	treeArray *current = NULL;
	treeArray *final = NULL;	

	current = root;
	//printf("Searching for %i.%i.%i.%i ", bin2dec(ip_addr[0]), bin2dec(ip_addr[2]), bin2dec(ip_addr[3]), bin2dec(ip_addr[4]));
		...
		while(n<256) {
			if(current->link[n] == NULL) {
				...
				if((current = (treeArray *)malloc(sizeof(treeArray))) != NULL){
					...
					if(i==prefix) {
						//printf("Tier %i entry added. Whole Address added\n", i);
						current->port = assdata_port;
						return;
					} else { // otherwise keep going
						//printf("Tier %i entry added. More to add...\n", i);
				...
				if(strcmp(current->link[n]->field, ip_addr[i-1]) == 0) {
					...
					if(i==prefix) {
						...
						//printf("Tier %i entry of address already exists. More to add now...\n", i);
						i++;
						current = current->link[n];
						break;
					}
...
// Temp test data
void testData() {
	int root1Branch, root2Branch, root3Branch, root4Branch, n1, n2, n3, n4, tempField;
	char tempBin[9];
	treeArray *prev = NULL;
	...
	root1Branch = rand() % 50;		// randomise for amount of branches from root
	for(n1=0;n1<root1Branch;n1++) {	// create that number and attach to root
		if((current = (treeArray *)malloc(sizeof(treeArray))) != NULL){
			...
			root2Branch = rand() % 5;		// randomise for amount of branches from tier 1
			for(n2=0;n2<root2Branch;n2++) {	// create that number and attach to tier 1 branch it spawned off
				if((current = (treeArray *)malloc(sizeof(treeArray))) != NULL){
					...
					root3Branch = rand() % 3;		// randomise for amount of branches from tier 2
					for(n3=0;n3<root3Branch;n3++) {	// create that number and attach to tier 2 branch it spawned off
						if((current = (treeArray *)malloc(sizeof(treeArray))) != NULL){
							...
							root4Branch = rand() % 2;		// randomise for amount of branches tier 3
