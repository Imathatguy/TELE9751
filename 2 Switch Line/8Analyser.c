...
which packets were dropped, percentage of success, etc
...
int main(void) {
	...
	int sockIN, recvMsgLen, n, count=1, high=0;
    ...
    analIN_addr.sin_port = htons(INPUT_PORT);			// Using port 5003
    ...
    while(1) {
    	...
    	if(count % 50 == 0) {
    		...
    		ratio = n / (recvPacket.sequenceNum * 1.0);	// timed by 1.0 to conver to floating point for division
