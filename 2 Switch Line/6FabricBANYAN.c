...
#define INPUT_PORT 50004
#define OUTPUT_PORT 50005
#define LOCAL_ADDRESS "127.0.0.1"
#define MAX_MSG_LEN 200

// create banyan fabric
...
int main(void) {
	...
	int n, start, sockIN, sockOUT, recvMsgLen, port, timer=100, count; //timer is 101 because first packet has to be between 0 and 99
    ...
    fabIN_addr.sin_port = htons(INPUT_PORT);		// Using port 5001
    ...
    fabOUT_addr.sin_port = htons(OUTPUT_PORT);					// output port 5002
	
    // Binding above data (address/port/etc) to socket descriptor sock for port 5001
	...
	while(1) {
		...
		//printf("Packet enters fabric from port %i (%s) at time %i\n", (recvPacket.fromPort), input, timer);
...
int banyan(char *out_addr, banyanElement *currentElement) {
	int stage, stageElements=4, port, blocked=0;		// find a way to count elements in each stage!!!!!!!! can only be 2, 4, 8, 16, 32 etc # ports
	// if element number is between 0 and 3, check addressPosition 0
	// if between 4 and 7, check addressPosition 1
	// if between 8 and 11, check addressPosition 2
	...
	if(out_addr[stage] == '0') {	// send from top node of element
		...
		if(currentElement -> topFlag == 0) {
			//printf("Output from element %i at top node ", currentElement->elementNumber);
			nextElement = currentElement->zero;
			currentElement -> topFlag = 1;
		} else {	// element node busy, packed dropped. blocked flag changed to 1
			//printf("Element top output busy, packet dropped.\n");
			blocked = 1;
		}
	} else {		// send from bottom node of element
		if(currentElement -> bottomFlag == 0) {
			//printf("Output from element %i at bottom node ", currentElement->elementNumber);
			nextElement = currentElement->one;
		} else {	// element node busy, packed dropped. blocked flag changed to 1
			//printf("Element bottom output busy, packet dropped.\n");
			blocked = 1;
		}
	}
